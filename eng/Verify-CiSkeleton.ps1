...
if ($buildUnitJob -notmatch [regex]::Escape('tests/DeviceRental.WebTests/DeviceRental.WebTests.csproj') -or
    $buildUnitJob -notmatch '(?s)DeviceRental\.WebTests\.csproj.*?--minimum-expected-tests\s+39' -or
    ...
    Add-Failure 'The build-unit job must require all 39 current Web tests and retain their TRX evidence.'
}
...
if ($integrationJob -notmatch [regex]::Escape('--filter Category=Database') -or
    $integrationJob -notmatch '(?s)--filter\s+Category=Database.*?--minimum-expected-tests\s+54') {
    Add-Failure 'The integration job must require all 54 current database and persistence contract cases.'
}
