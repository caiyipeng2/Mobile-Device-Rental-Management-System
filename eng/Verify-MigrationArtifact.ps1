...
if ($migrationFiles.Count -ne 4) {
    throw "Expected exactly four executable migrations, found $($migrationFiles.Count)."
}

$expectedNames = @('IdentityAndAccessPolicy', 'AuditAndOutbox', 'DeviceImages', 'LoanPolicyVersions')
...
Write-Host 'PASS: exactly four ordered migrations and the committed idempotent SQL artifact agree.'
