...
Development is in progress on `codex/mobile-device-rental-system`. The repository contains the approved specifications, responsive interface preview, .NET 10 modular monolith, identity application service, PostgreSQL identity/audit/outbox/device/loan/image/policy migrations, transactional EF device/loan/image-metadata/policy persistence, bounded JPEG/PNG/WebP validation, private filesystem image storage, an authenticated `/devices/{id}/image` proxy, and a clickable Razor Pages device desk. Configure `Storage:DeviceImageRoot` to a dedicated path outside the Web content root. With `Demo:Enabled=true`, the Web app serves synthetic preview data; with Demo disabled, authenticated device/loan reads, policy-versioned borrow, and core borrow/return operations use PostgreSQL. Production readiness still requires email verification/password recovery and Worker notifications.
