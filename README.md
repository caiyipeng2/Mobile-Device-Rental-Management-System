...
Development is in progress on `codex/mobile-device-rental-system`. The repository contains the approved specifications, responsive interface preview, polished device and account-entry UI, .NET 10 modular monolith, identity application service, PostgreSQL identity/audit/outbox/delivery/device/loan/image/policy migrations, transactional EF device/loan/image-metadata/policy persistence, recoverable Outbox claim/lease CAS transitions with loan version/due/return checks, AES-GCM notification payload encoding/decoding, versioned notification templates, TLS SMTP transport, transactional account-registration and loan-lifecycle event producers, advance/due reminder scheduling, pending-reminder cancellation, verification/password-reset resend events, per-attempt delivery history, bounded JPEG/PNG/WebP validation, private filesystem image storage, authenticated email-verification/password-reset token flows, an authenticated `/devices/{id}/image` proxy, and a clickable Razor Pages device desk. Configure `Storage:DeviceImageRoot` to a dedicated path outside the Web content root. With `Demo:Enabled=true`, the Web app serves synthetic preview data; with Demo disabled, authenticated device/loan reads, policy-versioned borrow, core borrow/return operations, and Identity token consumption use PostgreSQL. Production deployment still requires the hosted Outbox Worker, notification encryption key, public verification URL, and company SMTP secrets.
